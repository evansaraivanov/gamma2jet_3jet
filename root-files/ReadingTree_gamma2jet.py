...
finput1 = TFile.Open("/eos/user/w/wasu/Gammajet/gammajet_sherpa_incl.root")
t1 = finput1.Get("tree_photon")
#t2 = finput2.Get("tree_data")

for i in t1:
#	if i.pass_HLT_j400 == 1 :#and i.j1_is_truth_jet and i.j2_is_truth_jet:
        if i.jet_pt[0]/1000 > 40 and i.jet_pt[0]/1000 < 2000 and abs(i.jet_eta[0]) < 2.1 and len(i.jet_pt)>1 and abs(i.jet_eta[1]) < 2.1 and i.jet_pt[1]/1000 > 20:
            	i.jet_pt[0] = i.jet_pt[0]/1000
            	i.jet_pt[1] = i.jet_pt[1]/1000
            	pTbin1 = FindBinIndex(i.jet_pt[0], bins)
            	pTbin2 = FindBinIndex(i.jet_pt[1], bins)
            	label1 = GetJetType(i.jet_PartonTruthLabelID[0])
            	label2 = GetJetType(i.jet_PartonTruthLabelID[1])
               # etabin1 = FindBinIndex(i.jet_eta[0], bins)
               # label1 =  GetJetType(i.jet_PartonTruthLabelID[0])
...
foutput = TFile("gamma2jet_sherpa_py.root","recreate")
